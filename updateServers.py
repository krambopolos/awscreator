...
def execute_commands(ssh):
    """Exécute les commandes nécessaires sur le serveur connecté via SSH."""
    commands = [
    ...
    for command in commands:
        ...
        stdout.channel.recv_exit_status()  # Attendre que la commande soit terminée
...
def check_server_with_password(ip_address, password):
    ...
    try:
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect(ip_address, username='ubuntu', password=password, timeout=10)
        execute_commands(ssh)
        ssh.close()
        return True
    except Exception as e:
