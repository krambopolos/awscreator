...
def main():
    ...
    default_region = input("Please enter the default AWS region to deploy to (e.g., us-east-1): ").strip()
    if not default_region:
        default_region = "us-east-1"
        print("No region provided. Using default region: us-east-1")
    ...
    # Check if the current user has AdministratorAccess policy
    if check_current_user_has_admin(iam_client, user_name):
        print("Current user already has AdministratorAccess policy. Continuing with this user.")
        current_user, new_access_key, new_secret_key = user_name, None, None
    else:
        print("Current user does not have AdministratorAccess policy. Please ensure the user has the necessary permissions.")
        sys.exit(1)
    ...
    # Ajouter ici la sélection de la région par l'utilisateur
    selected_region = select_region_with_input(region_quotas, default_region)
    configure_aws_session(access_key, secret_key, selected_region)
    ...
    while regions_to_try:
        ...
        for instance_type in instance_types_priority:
            supported_zones = get_supported_instance_type_zones(selected_region, instance_type)
            if supported_zones:
        ...
        if available_instance_type:
            print(f"Selected instance type: {available_instance_type} in zone {availability_zone}")
        else:
            print(f"No available instance types found in {selected_region}. Trying another region...")
            if selected_region in regions_to_try:
                regions_to_try.remove(selected_region)
            if not regions_to_try:
                print("No regions left to try. Exiting.")
                sys.exit(1)
            selected_region = regions_to_try[0]
            configure_aws_session(access_key, secret_key, selected_region)
            continue
        ...
        if not subnet:
            ...
            supported_zones = get_supported_instance_type_zones(selected_region, 't2.micro')
        ...
        ami_id = get_ubuntu_ami(selected_region)
        if not ami_id:
        ...
        try:
            use_password = False
            if ssh_key_response:
                instances = launch_ec2_instances(boto3.resource('ec2', region_name=selected_region), ami_id, available_instance_type, key_name, security_group.id, subnet.id, instance_count)
                ...
                ssh_pwauth: true
                '''
                instances = launch_ec2_instances(boto3.resource('ec2', region_name=selected_region), ami_id, available_instance_type, None, security_group.id, subnet.id, instance_count, user_data=user_data)
            ...
            if "Blocked" in str(e):
                print(f"Account is blocked in region {selected_region}. Trying another region...")
                if selected_region in regions_to_try:
                    regions_to_try.remove(selected_region)
                if not regions_to_try:
                    print("No regions left to try. Exiting.")
    ...
    if deploy_bot == 'yes':
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = []
            for instance in instances:
                futures.append(executor.submit(deploy_bot_on_instance, instance, use_password, key_path_or_password, access_key, secret_key, selected_region, available_instance_type))
            
            for i, future in enumerate(tqdm(concurrent.futures.as_completed(futures), total=len(futures), desc="Deployments")):
                try:
                    result = future.result()
                    if result:
                        print(f"Deployment {i + 1}/{len(futures)} completed successfully.")
                    else:
                        print(f"Deployment {i + 1}/{len(futures)} failed.")
                except Exception as e:
                    print(f"Deployment {i + 1}/{len(futures)} generated an exception: {e}")
