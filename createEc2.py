...
def attach_admin_policy_to_user(iam_client, user_name):
    ...
    try:
        ...
        print(f"AdministratorAccess policy attached to user {user_name}.")
        return True
    except ClientError as e:
        print(f"Error attaching AdministratorAccess policy to user {user_name}: {e}")
        return False
...
def handle_instance_count(update: Update, context: CallbackContext) -> int:
    ...
    try:
        update.message.reply_text('Configuring AWS session...')
        configure_aws_session(access_key, secret_key, default_region)
    except Exception as e:
        update.message.reply_text(format_error_message(f"Failed to configure AWS session: {e}"))
        return ConversationHandler.END
    ...
    if check_current_user_has_admin(iam_client, user_name):
        ...
        update.message.reply_text(format_error_message("Current user does not have AdministratorAccess policy. Attaching the policy..."))
        if attach_admin_policy_to_user(iam_client, user_name):
            update.message.reply_text(format_success_message("AdministratorAccess policy attached successfully."))
        else:
            update.message.reply_text(format_error_message("Failed to attach AdministratorAccess policy. Exiting."))
            return ConversationHandler.END
    ...
    while regions_to_try:
        ...
        if available_instance_type:
            ...
            try:
                update.message.reply_text(f'Configuring AWS session for region {default_region}...')
                configure_aws_session(access_key, secret_key, default_region)
            except Exception as e:
                update.message.reply_text(format_error_message(f"Failed to configure AWS session for region {default_region}: {e}"))
                return ConversationHandler.END
            continue
        ...
        try:
            ...
            if not ami_id:
                update.message.reply_text(format_error_message(f"Failed to find a suitable Ubuntu 22.04 AMI in region {default_region}. Trying another region..."))
                if default_region in regions_to_try:
                    regions_to_try.remove(default_region)
                if not regions_to_try:
                    update.message.reply_text(format_error_message("No regions left to try. Exiting."))
            ...
            if not vpc:
                update.message.reply_text(f'No existing VPC found. Creating a new VPC in {default_region}...')
                vpc = create_vpc()
