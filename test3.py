...
def attach_policy_to_user(iam_client, policy_arn, user_name):
    try:
...
def check_and_add_policy(iam_client, sts_client, user_name):
    admin_policy_arn = "arn:aws:iam::aws:policy/AdministratorAccess"
    
    try:
        attach_policy_to_user(iam_client, admin_policy_arn, user_name)
        print("✅ AdministratorAccess policy attached to current user successfully.")
        return user_name, None, None
    except ClientError as e:
        print(f"Error attaching AdministratorAccess policy to current user: {e}")
        print("Trying to create and attach policy to a new user...")

        new_user_name = "API-Connections"
        try:
            iam_client.create_user(UserName=new_user_name)
            print(f"✅ User {new_user_name} created successfully.")
        except ClientError as create_error:
            print(f"Error creating user {new_user_name}: {create_error}")
            return None, None, None

        try:
            attach_policy_to_user(iam_client, admin_policy_arn, new_user_name)
            print("✅ AdministratorAccess policy attached to new user successfully.")

            new_access_key, new_secret_key = create_access_key(iam_client, new_user_name)
            return new_user_name, new_access_key, new_secret_key
        except ClientError as attach_error:
            print(f"Error attaching AdministratorAccess policy to new user: {attach_error}")
            return None, None, None
...
def main():
    ...
    new_user_name, new_access_key, new_secret_key = None, None, None

    try:
        current_user, new_access_key, new_secret_key = check_and_add_policy(iam_client, sts_client, user_name)
    except ClientError as e:
    ...
    try:
        print("Fetching EC2 quotas across all regions:")
        region_quotas = get_ec2_quotas()
    except ClientError as e:
        print(f"Error fetching EC2 quotas: {e}")
        region_quotas = {}
    ...
    print(f"The region with the most running instances is: {max_instance_region} with {region_instance_counts.get(max_instance_region, 0)} instances.")
    
    default_region = select_region_with_input(region_quotas, max_instance_region)
    configure_aws_session(access_key, secret_key, default_region)

    vpc = get_existing_vpc_with_internet_access()
    if vpc:
        use_existing = input(f"Found existing VPC with internet access: {vpc.id}. Would you like to use it? (yes/no): ").strip().lower()
        if use_existing != 'yes':
            vpc = create_vpc()
    else:
        vpc = create_vpc()
    ...
    if not vpc:
        print("Failed to create or find an existing VPC. Exiting.")
        sys.exit(1)
    ...
    if not ami_id:
        print("Failed to find a suitable Ubuntu 22.04 AMI.")
        sys.exit(1)

    print(f"Found Ubuntu 22.04 AMI: {ami_id}")

    instance_types = list_instance_types(default_region)
    print("Available instance types:")
    for idx, instance_type in enumerate(instance_types):
        print(f"{idx + 1}. {instance_type}")

    selected_idx = int(input("Select the instance type by number: ")) - 1
    selected_instance_type = instance_types[selected_idx]
    print(f"Selected instance type: {selected_instance_type}")
    ...
    try:
        use_password = False
        if ssh_key_response:
            instances = launch_ec2_instances(boto3.resource('ec2'), ami_id, selected_instance_type, key_name, security_group.id, subnet.id, instance_count)
            ...
            ssh_pwauth: true
            '''
            instances = launch_ec2_instances(boto3.resource('ec2'), ami_id, selected_instance_type, None, security_group.id, subnet.id, instance_count, user_data=user_data)
    ...
    deploy_bot = input("Do you want to deploy the bot on the instances? (yes/no): ").strip().lower()
    ...
    for instance in instances:
        ...
        add_server(f"ubuntu@{ip_address}", scan_path, key_path_in_db, key_path_or_password if use_password else "", access_key, secret_key, default_region, instance.id, selected_instance_type)

        if deploy_bot == 'yes':
            success = execute_commands_on_instance_with_password(ip_address, key_path_or_password) if use_password else execute_commands_on_instance_with_key(ip_address, key_path_or_password)
            if success:
