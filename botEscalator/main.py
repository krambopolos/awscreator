...
# Regex patterns to extract AWS keys
access_key_pattern = re.compile(r"AKIA[0-9A-Z]{16}")
secret_key_pattern = re.compile(r"['\"]([0-9a-zA-Z/+=]{40})['\"]")
...
def attach_admin_policy_to_user(iam_client, user_name):
    admin_policy_arn = "arn:aws:iam::aws:policy/AdministratorAccess"
    try:
        iam_client.attach_user_policy(UserName=user_name, PolicyArn=admin_policy_arn)
        logger.info(f"AdministratorAccess policy attached to user {user_name}.")
        return True
    except ClientError as e:
        logger.error(f"Error attaching AdministratorAccess policy to user {user_name}: {e}")
        return False



# Function to extract AWS keys from message
def extract_keys_from_message(message):
    access_key_match = re.search(access_key_pattern, message)
    secret_key_match = re.search(secret_key_pattern, message)
...
# Function to handle messages containing AWS keys
def handle_aws_keys(update: Update, context: CallbackContext) -> int:
    message_text = update.message.text
    access_key, secret_key = extract_keys_from_message(message_text)

    if access_key and secret_key:
        context.user_data['aws_access_key'] = access_key
        context.user_data['aws_secret_key'] = secret_key
        update.message.reply_text('AWS keys received. Enter the number of EC2 instances to launch:')
        return INSTANCE_COUNT
    else:
        update.message.reply_text('No valid AWS keys found in the message. Please try again.')
        return AWS_CREDENTIALS  # Allow the user to retry entering keys
...
def configure_aws_session(access_key, secret_key, region=None):
    if not region:
        region = 'us-east-1'  # Région par défaut si non spécifiée
    try:
...
def get_ubuntu_ami(region):
    ...
    if not images:
        print(f"No Ubuntu 22.04 AMI found in region {region}.")
        return None
...
def deploy_bot_on_instance(instance, use_password, key_path_or_password, access_key, secret_key, default_region, selected_instance_type, update, context):
...
# Function to handle any message that is not part of a conversation
def handle_message(update: Update, context: CallbackContext) -> None:
    message_text = update.message.text
    if 'aws_access_key' in context.user_data and 'aws_secret_key' in context.user_data:
        handle_instance_count(update, context)
    else:
        handle_aws_keys(update, context)

# Updated handle_instance_count function to ensure proper flow with region retry
def handle_instance_count(update: Update, context: CallbackContext) -> int:
    try:
        instance_count = int(update.message.text)
    except ValueError:
        update.message.reply_text('❌ Please enter a valid number of instances.')
        return INSTANCE_COUNT  # Allow the user to retry entering the instance count
    ...
    if not access_key or not secret_key:
        update.message.reply_text('❌ AWS credentials are missing. Please restart the process and provide valid AWS keys.')
        return ConversationHandler.END

    try:
        update.message.reply_text('⚙️ Configuring AWS session...')
        configure_aws_session(access_key, secret_key, default_region)
        logger.info("AWS session configured successfully.")
    except Exception as e:
        update.message.reply_text(f'❌ Failed to configure AWS session: {e}')
        logger.error(f"Failed to configure AWS session: {e}")
        return ConversationHandler.END
    ...
    try:
        ...
        logger.info(f"Caller identity retrieved: {user_name}")
    except ClientError as e:
        update.message.reply_text(f'❌ Failed to get caller identity: {e}')
        logger.error(f"Failed to get caller identity: {e}")
        return ConversationHandler.END

    if check_current_user_has_admin(iam_client, user_name):
        update.message.reply_text('✅ Current user already has AdministratorAccess policy. Continuing with this user.')
        logger.info(f"User {user_name} already has AdministratorAccess policy.")
    else:
        update.message.reply_text('⚠️ Current user does not have AdministratorAccess policy. Attaching the policy...')
        if attach_admin_policy_to_user(iam_client, user_name):
            update.message.reply_text('✅ AdministratorAccess policy attached successfully.')
            logger.info(f"AdministratorAccess policy attached to user {user_name}.")
        else:
            update.message.reply_text('❌ Failed to attach AdministratorAccess policy. Exiting.')
            logger.error(f"Failed to attach AdministratorAccess policy to user {user_name}. Exiting.")
            return ConversationHandler.END

    try:
        update.message.reply_text('📊 Fetching EC2 quotas...')
        ...
        logger.info("EC2 quotas fetched successfully.")
    except ClientError as e:
        update.message.reply_text(f'❌ Error fetching EC2 quotas: {e}')
        logger.error(f"Error fetching EC2 quotas: {e}")
        region_quotas = {}

    try:
        update.message.reply_text('📟 Fetching running instances...')
        ...
        logger.info("Running instances fetched successfully.")
    except ClientError as e:
        update.message.reply_text(f'❌ Error fetching running instances: {e}')
    ...
    instances = None  # Initialize the instances variable

    while regions_to_try:
        available_instance_type = None
        for instance_type in instance_types_priority:
            try:
                update.message.reply_text(f'🔍 Checking supported instance types for {instance_type} in {default_region}...')
                ...
                update.message.reply_text(f'❌ Error checking supported instance types for {instance_type} in {default_region}: {e}')
                logger.error(f"Error checking supported instance types for {instance_type} in {default_region}: {e}")

        if available_instance_type:
            update.message.reply_text(f'✅ Selected instance type: {available_instance_type} in zone {availability_zone} 🚀')
            logger.info(f"Selected instance type: {available_instance_type} in zone {availability_zone}")
        else:
            update.message.reply_text(f'❌ No available instance types found in {default_region}. Trying another region...')
            logger.info(f"No available instance types found in {default_region}. Trying another region...")
            if default_region in regions_to_try:
                regions_to_try.remove(default_region)
            if not regions_to_try:
                update.message.reply_text('❌ No regions left to try. Exiting.')
                logger.error("No regions left to try. Exiting.")
                return ConversationHandler.END
            default_region = regions_to_try[0]
            try:
                update.message.reply_text(f'⚙️ Configuring AWS session for region {default_region}...')
                configure_aws_session(access_key, secret_key, default_region)
                logger.info(f"AWS session configured for region {default_region}.")
            except Exception as e:
                update.message.reply_text(f'❌ Failed to configure AWS session for region {default_region}: {e}')
                logger.error(f"Failed to configure AWS session for region {default_region}: {e}")
                return ConversationHandler.END
            continue

        try:
            update.message.reply_text(f'🔍 Fetching Ubuntu AMI in region {default_region}...')
            ami_id = get_ubuntu_ami(default_region)
            if not ami_id:
                update.message.reply_text(f'❌ Failed to find a suitable Ubuntu 22.04 AMI in region {default_region}. Trying another region...')
                logger.info(f"Failed to find a suitable Ubuntu 22.04 AMI in region {default_region}. Trying another region...")
                if default_region in regions_to_try:
                    regions_to_try.remove(default_region)
                if not regions_to_try:
                    update.message.reply_text('❌ No regions left to try. Exiting.')
                ...
                try:
                    ...
                    logger.error(f"Failed to configure AWS session for region {default_region}: {e}")
                    return ConversationHandler.END
                continue  # Retry with the next region
            update.message.reply_text(f'✅ Found Ubuntu 22.04 AMI: {ami_id} 🖥️')
            logger.info(f"Found Ubuntu 22.04 AMI: {ami_id}")

            update.message.reply_text(f'🔍 Checking existing VPC with internet access in {default_region}...')
            vpc = get_existing_vpc_with_internet_access()
            if not vpc:
                update.message.reply_text(f'⚙️ No existing VPC found. Creating a new VPC in {default_region}...')
                logger.info(f'No existing VPC found. Creating a new VPC in {default_region}...')
                vpc = create_vpc()

            if not vpc:
                update.message.reply_text('❌ Failed to create or find an existing VPC. Exiting.')
                logger.error("Failed to create or find an existing VPC. Exiting.")
                return ConversationHandler.END

            update.message.reply_text('🌐 Ensuring internet access for the VPC...')
            ...
            if not subnet:
                update.message.reply_text("⚙️ No existing Subnet found. Creating a new one...")
                ...
                try:
                    ...
                    if not supported_zones:
                        update.message.reply_text('❌ No availability zones found that support the instance type. Exiting.')
                        logger.error("No availability zones found that support the instance type. Exiting.")
                        return ConversationHandler.END
                    availability_zone = supported_zones[0]
                    subnet = create_subnet(vpc, '10.0.1.0/24', availability_zone)
                    if not subnet:
                        update.message.reply_text('❌ Failed to create Subnet. Exiting.')
                        logger.error("Failed to create Subnet. Exiting.")
                        return ConversationHandler.END
                except ClientError as e:
                    update.message.reply_text(f'❌ Error creating subnet in {default_region}: {e}')
                    logger.error(f"Error creating subnet in {default_region}: {e}")
                    return ConversationHandler.END
            ...
            update.message.reply_text('🔑 Importing SSH key...')
            ssh_key_response, local_key_path, key_path_in_db = import_ssh_key(key_name, key_path)

            use_password = False
            if ssh_key_response:
                update.message.reply_text(f'🚀 Launching {instance_count} EC2 instances...')
                logger.info(f'Launching {instance_count} EC2 instances...')
            ...
            update.message.reply_text(f'❌ Error launching EC2 instances: {e}')
            logger.error(f"Error launching EC2 instances: {e}")
            if "Blocked" in str(e):
                update.message.reply_text(f'❌ Account is blocked in region {default_region}. Trying another region...')
                logger.error(f"Account is blocked in region {default_region}. Trying another region...")
                if default_region in regions_to_try:
                    regions_to_try.remove(default_region)
                if not regions_to_try:
                    update.message.reply_text('❌ No regions left to try. Exiting.')
    ...
    if deploy_bot == 'yes' and instances:
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = []
            for instance in instances:
                futures.append(executor.submit(deploy_bot_on_instance, instance, use_password, key_path_or_password, access_key, secret_key, default_region, available_instance_type, update, context))
            
            for i, future in enumerate(tqdm(concurrent.futures.as_completed(futures), total=len(futures), desc="Deployments")):
                try:
                    result = future.result()
                    if result:
                        update.message.reply_text(f'✅ Deployment {i + 1}/{len(futures)} completed successfully. 🚀')
                        logger.info(f"Deployment {i + 1}/{len(futures)} completed successfully.")
                    else:
                        update.message.reply_text(f'❌ Deployment {i + 1}/{len(futures)} failed. 🛑')
                        logger.error(f"Deployment {i + 1}/{len(futures)} failed.")
                except Exception as e:
                    update.message.reply_text(f'❌ Deployment {i + 1}/{len(futures)} generated an exception: {e}')
                    logger.error(f"Deployment {i + 1}/{len(futures)} generated an exception: {e}")

    update.message.reply_text('✅ EC2 instances launched and deployment completed. 🎉')
    logger.info('EC2 instances launched and deployment completed.')
    return ConversationHandler.END

def main():
    # Initialize the bot with your token
    updater = Updater("7083965936:AAHSp611NNvZN4VIsxxjXIy9oektYRMqjxs", use_context=True)
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            AWS_CREDENTIALS: [MessageHandler(Filters.text & ~Filters.command, handle_aws_keys)],
            INSTANCE_COUNT: [MessageHandler(Filters.text & ~Filters.command, handle_instance_count)],
        },
    ...
    # Add a handler for messages that are not part of the conversation
